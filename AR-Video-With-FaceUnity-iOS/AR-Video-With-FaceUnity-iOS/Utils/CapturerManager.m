...
- (void)didOutputPixelBuffer:(CVPixelBufferRef)pixelBuffer frameTime:(CMTime)time {
    if ([self.delegate respondsToSelector:@selector(processFrame:)]) {
        ...
        [self.consumer consumePixelBuffer:outputPixelBuffer withTimestamp:time rotation:ARVideoRotationNone];
    }
}
...
- (void)didOutputVideoFrame:(id<AGMVideoFrame>)frame {
    if ([frame isKindOfClass:AGMCVPixelBuffer.class]) {
        if ([self.delegate respondsToSelector:@selector(processFrame:)]) {
            ...
            if (!outputPixelBuffer) return;
            [self.consumer consumePixelBuffer:outputPixelBuffer withTimestamp:CMTimeMake(CACurrentMediaTime() * 1000, 1000) rotation:ARVideoRotationNone];
            if (self.videoView) {
        ...
        if (!outputPixelBuffer) return;
        [self.consumer consumePixelBuffer:outputPixelBuffer withTimestamp:CMTimeMake(1, (int32_t)frame.timeStampMs) rotation:ARVideoRotationNone];
        if (self.videoView) {
